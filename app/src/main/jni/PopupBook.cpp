...
BasePopupBook* createES3Popupbook() {
    PopupBook* popupBook = new PopupBook();
    if (!popupBook->init()) {
        delete popupBook;
        return NULL;
    }
...
PopupBook::PopupBook():   mEglContext(eglGetCurrentContext()),
                          mProgram(0),
                          mVBState(0)
{
...
void PopupBook::unmapOffsetBuf() {
    glUnmapBuffer(GL_ARRAY_BUFFER);
}

float* PopupBook::mapTransformBuf() {
    glBindBuffer(GL_ARRAY_BUFFER, mVB[VB_SCALEROT]);
    return (float*)glMapBufferRange(GL_ARRAY_BUFFER,
...
void PopupBook::unmapTransformBuf() {
    glUnmapBuffer(GL_ARRAY_BUFFER);
}

void PopupBook::draw(unsigned int numInstances) {
